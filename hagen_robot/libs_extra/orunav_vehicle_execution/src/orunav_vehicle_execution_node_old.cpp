...
void KMOVehicleExecutionNode::goalTrajectoryCallback(const move_base_msgs::MoveBaseActionGoalConstPtr& msg){
    ...
    if(path_out.size()>3){
      ...
      // current_smoothed_path.goal_id = 1;
      // current_smoothed_path.target_start = current_pose_path;
      // current_smoothed_path.target_goal = goal_;
      // orunav_msgs::RobotTarget target;
      orunav_msgs::PoseSteering ps;
      auto  start_pose2d = current_smoothed_path.getPose2d(0);
      ps.pose = createMsgFromPose2d(start_pose2d);
      ps.steering = current_smoothed_path.getSteeringAngle(0);
      current_target_.start = ps;
      auto goal_pose2d = current_smoothed_path.getPose2d(current_smoothed_path.sizePath()-1);
      ps.pose = createMsgFromPose2d(start_pose2d);
      ps.steering = current_smoothed_path.getSteeringAngle(current_smoothed_path.sizePath()-1);
      ...
      // vehicle_state_.
      calculateCommands(current_smoothed_path);
    }
